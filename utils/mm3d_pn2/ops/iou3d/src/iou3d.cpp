...
/*
Исправленная версия для Windows (MSVC)
Changes: Replaced Variable Length Arrays (VLA) with std::vector
*/
...
int nms_gpu(at::Tensor boxes, at::Tensor keep, float nms_overlap_thresh){
    ...
    // --- FIX START ---
    // Было: unsigned long long mask_cpu[boxes_num * col_blocks];
    std::vector<unsigned long long> mask_cpu(boxes_num * col_blocks);
    // -----------------

    CHECK_ERROR(cudaMemcpy(&mask_cpu[0], mask_data, boxes_num * col_blocks * sizeof(unsigned long long), cudaMemcpyDeviceToHost));
    cudaFree(mask_data);

    // --- FIX START ---
    // Было: unsigned long long remv_cpu[col_blocks];
    std::vector<unsigned long long> remv_cpu(col_blocks, 0); // инициализируем нулями
    // -----------------
